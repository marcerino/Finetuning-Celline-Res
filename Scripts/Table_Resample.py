...
def atomicstats(
    path: str, Model: str = "Unkonwn", Samples: str = None, Run: str = None
) -> pl.DataFrame:
    df = pl.read_csv(path)
    df = df.select(["metric", "value"]).transpose(column_names="metric")
    df = pl.concat(
        [pl.DataFrame({"Model": Model, "Samples": Samples, "Run": Run}), df],
        how="horizontal",
    )
    return df
...
def totaltable(path_to_dir: str) -> pl.DataFrame:
    ...
    dataframes = [
        atomicstats(argset["path"], argset["Model"], argset["Samples"], argset["Run"])
        for argset in stats_and_args
    ]
    print("Concating")
